...
if LINKS.get('/payment'):               # the form's "Preview only" note gets a link to the payment page preview
    html = html.replace('</head>', '<script>window.__PREVIEW_LINKS = ' + json.dumps({'payment': LINKS['/payment']}) + ';</script>\n</head>', 1)   # the real <body> starts a line (the word also appears inside inlined scripts)
